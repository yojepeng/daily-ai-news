...
"""
每日科技 推送
流程: 分板块抓取 RSS -> 各板块用 DeepSeek 浓缩成 60-80 字一句话新闻 -> Server酱 推送到微信
...
def summarize_topic(section: str, n: int, items):
    """调用 DeepSeek 把某板块新闻浓缩成 60-80 字的一句话新闻列表。"""
    if not items:
    ...
    prompt = (
        f"你是科技资讯编辑。针对「{section}」的今日新闻标题，产出「一句话新闻」列表。要求：\n"
        "1. 不要板块开头概述，不要每条的小标题或加粗标题。\n"
        "2. 每条仅输出一句话（约 60-80 个汉字），说清核心事实：主体 + 做了什么 + 关键数据/影响；不展开分析、不评论。\n"
        f"3. 最多 {n} 条，不足按实际条数；按重要性排序；相近内容合并为一条。\n"
        "4. 剔除纯营销软文、无实质信息的水稿。\n"
        "5. 输出每条一行，用「1. 」「2. 」编号，不带 Markdown 加粗/链接等标记。\n\n"
        f"新闻列表：\n{news_text}"
    )
