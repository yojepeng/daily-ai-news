...
关机也能跑：由 GitHub 的服务器在**每天北京时间 06:30** 自动执行，分 4 个板块抓取科技资讯 → 各板块用 DeepSeek 总结成详细报道 → Server酱推送到你的微信。你的电脑无需开机。

推送内容分 4 个板块，每个板块各 8 条：
- **一、科技新闻**
- **二、Vibe Coding**
- **三、AI 应用**
- **四、AI 动向**
...
- **想换/加新闻源或板块？** 编辑 `push_news.py` 顶部的 `TOPICS` 列表（每个板块的 `section` 名称与 `feeds` 搜索词）。想调整每板块条数改 `ITEMS_PER_TOPIC`（默认 8）。
...
> 相关参数：每板块条数由环境变量 `ITEMS_PER_TOPIC`（默认 8）控制；新闻时间窗口由 `HOURS_BACK`（默认 48 小时）控制。
