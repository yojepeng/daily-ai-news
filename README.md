...
关机也能跑：由 GitHub 的服务器在**每天北京时间 06:30** 自动执行，分 4 个板块抓取科技资讯 → 各板块用 DeepSeek 浓缩成极简「一句话新闻」→ Server酱推送到你的微信（可选附**语音播报**链接）。你的电脑无需开机。

推送内容分 4 个板块（共 18 条，每条一句话）：
- **一、Vibe Coding**（4 条）
- **二、OPC 一人公司**（4 条，政策 / 热点 / 新闻）
- **三、AI**（5 条，应用 + 动向 + 模型发布）
- **四、科技新闻**（5 条）
