...
推送内容分 4 个板块（共 28 条，每条为 60-80 字的一句话新闻，无小标题）：
- **一、Vibe Coding**（6 条）
...
对推送内容做了「精简 + 省 token」两项优化：

1. **内容压缩为「一句话新闻」**：DeepSeek 提示词由"每条 2-4 句话详细报道 + 开头概述"改为"每条 ≤35 字一句客观陈述，无概述、无评论、相近合并"。输出从 Markdown 段落变为极简编号列表，单条信息密度更高、字数大幅下降。
2. **整体减量**：各板块条数由 6/6/8/8（共 28 条）下调为 **4/4/5/5（共 18 条）**；同时新闻输入列表去掉「来源」字段（一句话新闻无需来源），进一步减少输入 token。
3. **省 token 效果**：输出条数 −36%、单条字数从 ~60-80 字压到 ≤35 字、且去掉开头概述；输入侧移除来源行。综合输出 token 预计降低约 70%，成本与微信阅读负担都明显减轻。

> 相关参数：每板块条数由各板块 `n` 控制（当前 4/4/5/5）；一句话上限在 `summarize_topic` 的 prompt 中为 35 字；如需更短/更长改该值即可。

---

### v1.0.0（2026-08-13，回退调整）

实测上一版「一句话新闻 + 18 条」过于精简，本次回退到原始「详细报道 + 28 条」形态，并正式引入版本号（`push_news.py` 顶部 `VERSION = "1.0.0"`）。具体：

1. **条数恢复 28 条**：各板块 `n` 由 4/4/5/5 调回 **6/6/8/8**。
2. **内容恢复详细报道**：DeepSeek 提示词改回"每条 2-4 句话（约 60-80 字）具体说明事实与影响 + 开头概述"，输出恢复 Markdown 段落样式；新闻输入列表恢复「来源」字段（给模型更多上下文）。
3. **保留 v0 阶段的稳定性改进**（不回退）：fallback 列表标题不重复、来源友好显示（Google News→「Google 新闻」）、DeepSeek 失败时在微信标题标「⚠️ AI总结失败」并列出具体原因。

> 说明：本项目此前无显式版本号，自本次起以语义化版本管理；当前稳定形态定为 v1.0.0。如需再调精简程度，改 `summarize_topic` 的 prompt 与各板块 `n` 即可。

---

### v1.1.0（2026-08-13，再次调整格式）

v1.0.0 的「详细报道 + 每条 2-4 句长段落 + 加粗小标题」实测过长（单条约 232 字），v1.1.0 改为介于「一句话极简（18 条/≤35 字）」与「详细报道」之间的形态：

1. **条数保持 28 条**（不变）。
2. **每条改为 60-80 字的一句话新闻**：DeepSeek 提示词调整为「每条仅一句、约 60-80 汉字、说清核心事实（主体+动作+关键数据/影响）、不展开分析」；**去掉板块开头概述**与**每条的加粗小标题**，输出为纯编号列表。
3. 版本号 `VERSION` 由 `1.0.0` 升到 `1.1.0`。
4. 此前的稳定性改进（fallback 标题去重、来源友好显示、DeepSeek 失败标「⚠️ AI总结失败」）继续保留。

> 说明：如需再调整单条字数，改 `summarize_topic` 的 prompt 中「60-80 个汉字」即可。
